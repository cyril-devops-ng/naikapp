                                        ...
                                        <tbody id="tableBody">
                                            <?php 
                                                ...
                                                foreach( $_SESSION['stockraw'] as $k=>$v){
                                                    $date = date('d-m-Y H:i:s', strtotime($v['upload_date']));
                                                    $d =  explode(' ',$date)[0] ;
                                                    $mth = explode('-',$d)[1];
                                                   echo '<tr>'.
                                                            '<td>'.$i++.'</td>'.
                                                            ...
                                                            '<td class="hidden-xs">'.$v['cost_price'].'</td>'.
//                                                            '<td class="hidden-xs">'.$v['selling_price'].'</td>'.
                                                            '<td class="hidden-xs" id="salesarea">'.$v['sales_area'].'</td>'.
                                                            '<td class="hidden-xs">'.explode('-',$v['upload_date'])[2].'-'.explode('-',$v['upload_date'])[1].'-'.explode('-',$v['upload_date'])[0].'</td>'.
                                                            '<td>'.$mmmm[intval($mth) - 1 ].'</td>'.
                                                            '<td class="hidden-xs">' .
                                                                '<button data-toggle="button" class="btn btn-sm btn-primary edit" id="' . $v['stock_name'] . '"> Edit </button>' .
                                                                '<button data-toggle="button" class="btn btn-sm btn-warning delete" id="' . $v['stock_name'] . '"> Delete </button></td>';
                                                           '</tr>'; 
                                                }
                                            ?>
                                        </tbody>
            ...
            $(document).ready(function () {
                ...
                $('body').on('click', '.delete', function () {
                    ...
                    if (c) {
                        ...
                        $.post('viewstock', {pleasedelete: true, stock: stock, salesarea:$('#salesarea').html()}, function (msg, stat, xhr) {
                            if (xhr.readyState === 4) {
                                if (xhr.status === 200) {
