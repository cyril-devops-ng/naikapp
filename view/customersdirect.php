...
<html>
    ...
    <head>
        <title>NAIK FEEDS Customers</title>
                                                ...
                                                <table class="table table-striped display" id="example">
                                                    ...
                                                    <tbody id="tableBody">
                                                        ...
                                                        foreach ($_SESSION['customers'] as $k => $v) {
                                                            ...
                                                            '<td class="hidden-xs">' . $v['customer_category'] . '</td>' .
                                                            '<td class="hidden-xs"><button data-toggle="button" class="btn btn-sm btn-danger delete" id="' . $v['cust_id'] . '"> Delete </button></td>' .
                                                            '</tr>';
                                                        }
                                                        ?>
                                                    </tbody>
            ...
            $(document).ready(function () {
                        ...
                        if (xhr.readyState === 4) {
                            if (xhr.status === 200) {
                                if (JSON.parse($.trim(msg)) === 'success') {
                                    setTimeout(function () {
                                        window.location.href = 'customersdirect';
                                    }, 2000);
                                }
                            }
                        }
                ...
                $('body').on('click', '.delete', function () {
                    ...
                    if (confirm("Are you sure you want to delete?")) {
                        $.post('customersdirect', {'cust_id': cust_id, 'delete': true}, function (msg, stat, xhr) {
                            if (xhr.readyState === 4) {
                                if (xhr.status === 200) {
