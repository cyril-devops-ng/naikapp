...
<head>
<title>NAIK FEEDS Stock Movement</title>
        ...
        <div class="col-lg-12">
          <div class="widget">
            <div class="widget-header"> <i class="icon-table"></i>
              <h3>STOCK CONFIRMATION</h3>
            </div>
            <div class="widget-content">
                <?php //rsort($_SESSION['requestlist']); ?>
                <div class="example_alt_pagination">
                  ...
                  <tbody>
                      ...
                      $i = 0;
//                        print '<pre>';print_r()
                        rsort($_SESSION['requestlist']);
                        foreach( $_SESSION['requestlist'] as $k=>$v){
                            if($v['status'] == 'Rejected'){
                                $img = '<img src="img/rejected.png" width="50px" height="5px">&nbsp;';
                            }
                            if($v['status'] == 'Pending'){
                                $img = '<img src="img/pending.png" width="50px" height="5px">&nbsp;';
                            }
                            if($v['status'] == 'Approved'){
                                $img = '<img src="img/approved.png" width="50px" height="5px">&nbsp;';
                            }
                            if($v['status'] == 'In Transit'){
                                $img = '<img src="img/transit.png" width="50px" height="5px">&nbsp;';
                            }
                            if($v['status'] == 'Completed'){
                                $img = '<img src="img/complete.png" width="50px" height="5px">';
                            }
                            $i+=1;
                             echo '<tr>';
                            echo '<td>' . $i . '</td>';
                            ...
                            echo '<td>'.$v['status'].'</td>';
                            if($v['status'] == 'In Transit'){
                                echo '<td class="hidden-xs"><button class="btn btn-sm btn-success confirm " id="'.$v['request_no'].'"> Confirm Receipt </button><img class="preloader" src="img/preload.gif" width="30px" height="25px" id="preloader-'.$v['request_no'].'">&nbsp;';
                                echo '<button class="btn btn-sm btn-danger reject" id="'.$v['request_no'].'"> Reject </button>&nbsp;';
                            }else{
                                echo '<td class="hidden-xs"><button data-target="#myModal" data-toggle="modal" class="btn btn-sm btn-primary view" id="view-'. $v['request_no'] .'"> View Details </button>&nbsp;';
                            }
                        }
...
<script>
    $(document).ready(function(){
        ...
        $('body').on('click','.confirm',function(){
            ...
            $('#preloader-'+$(this).attr('id')).show();
            $.post('stockmovement',{request:$(this).attr('id')},function(msg,stat,xhr){
                if(xhr.readyState === 4){
                    if(xhr.status === 200){
                        var m = JSON.parse($.trim(msg));
                        if(JSON.parse($.trim(msg))){
                            setTimeout(function(){
//                                alert($.trim(msg));
//                                $('#preloader-'+$(this).attr('id')).hide();
                                window.location.href = 'stockmovement';
                            },2000);
                        }
                    }
                }
        ...
        $('body').on('click','.reject',function(){
            ...
            if ( confirm("Are you sure you want to reject this? ")){
                $.post('stockmovement',{rejectrequest:$(this).attr('id')},function(msg,stat,xhr){
                    if(xhr.readyState === 4){
                        if(xhr.status === 200){
                            window.location.href = 'stockmovement';
                         }
        ...
        $('body').on('click','.view',function(){
            var requestno = $(this).attr('id').split('-')[1];
            
            $.post('stockmovement',{modal:true,'requestno':requestno},function(msg,stat,xhr){
                if(xhr.readyState === 4){
                    if(xhr.status === 200){
