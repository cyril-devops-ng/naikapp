...
<html>
    ...
    <head>
        <title>NAIK FEEDS Pending Request List</title>
                                    ...
                                    <table class="table table-striped">
                                        <thead>
                                            <tr style="font-size:20px;">
                                                ...
                                                <th class="hidden-xs">Quantity Requested</th>
                                                <th class="hidden-xs">Request Date</th>
                                                <th class="hidden-xs">Request Status</th>
                                                <th class="hidden-xs">Requested From</th>
                                                <th class="hidden-xs">Requested By</th>
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($_SESSION['requestlist'] as $k => $v) {
                                                    if($v['status'] == 'Pending'){
                                                        $img = '<img src="img/pending.png" width="40px" height="30px">&nbsp;';
                                                       $i+=1;
                                                        echo '<tr>';
                                                        echo '<td>' . $i . '</td>';
                                                        ...
                                                        if($v['status'] == 'Pending'){
                                                                ...
                                                                '<button  data-target="#myModal" data-toggle="modal" class="btn btn-sm btn-danger reject" id="' . $v['request_no'] . '"> Reject </button></td>';
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                var perform = function (request, action) {
                    $.post('myrequestlist', {'requestno': request, 'action': action , 'comments':$('#commentText').val()}, function (msg, stat, xhr) {
                        if (xhr.readyState === 4) {
                            if (xhr.status === 200) {
                                if (JSON.parse($.trim(msg)) === 'success') {
                                    setTimeout(function () {
                                        window.location.href = 'pendingrequest';
                                    }, 2000);
                                }
                            }
                        }
                ...
                $('body').on('click', '.reject', function () {
                    var request = $(this).attr('id');
                    window.reject = request;
//                    alert(request);
                    //perform(request, 'reject');
                });
                $('body').on('click','#saveReject',function(evt){
                    evt.preventDefault();
                    perform(window.reject,'reject');
                });
